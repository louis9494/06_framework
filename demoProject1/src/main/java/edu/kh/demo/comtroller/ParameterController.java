...
import edu.kh.demo.medel.dto.MemberDTO;
...
public class ParameterController {
	 ...
	 * 매개변수 자리에 원하는 객체를 작성을 하면
	 * 이미 만들어서 존재하는 객체를 바인딩 또는 없으면 생성해서 바인딩
	 * 
	 * --> ArqumentResolver (전달 인자 해결사) 가 도와줌
	 * */
	
	@PostMapping("test1") 		// /param/test1
	public String paramTest1(HttpServletRequest req) {
			
		String inputName = req.getParameter("inputName");//키 -> req로 리턴
	 ...
	 * 
	 * [속성 추가 작성법]
	 * @RequestParam(value="name", required="false"-> 필수로 받아야 하는 변수냐, defaultValue="1")
	 * value : 전달받은 input 태그의 name 속성값 (파라미터 key)
	 * 
	 * required : 입력된 name 속성값 파라미터 필수 여부 지정(기본값 true)+
	 * -> required = true인 파라미터가 존재하지 않는다면 400 Bad Request 에러 발생
	 * 
	 * defaultValues : 파라미터 중 일치하는 name 속성값이 없을 경우에 대입할 값 지정.
